...
exports.addToFile = function(filename,lineToAdd,beforeMarker){
    try {
        ...
        if (indexOf === -1 || !beforeMarker) {
            //just add it to end of file
            if (fileSrc.endsWith('\n')) {
                fileSrc += lineToAdd + '\n';
            } else {
                fileSrc += '\n' + lineToAdd + '\n';
...
exports.processTemplates = function(name,dir,type,that,defaultDir,configName,module){
    ...
    if (_.isEmpty(that.config.getAll())) {
        throw new Error('This does not appear to be a valid cg-angular15 project!  Cannot find correct config in .yo-rc.json.');
    }
    ...
    _.chain(fs.readdirSync(templateDirectory))
        ...
        .each(function(template){
            ...
            if (type === 'module' && template.startsWith('module')) {
                //special case the module.js and module.scss files that
                //shouldnt be injected into the parent module cause theyre templated inside
                //the index.js and index.scss
                return;
            }
...
exports.injectComponentRoute = function(moduleFile,uirouter,name,route,that){
    ...
    if (uirouter){
        ...
        exports.addToFile(moduleFile,'$routeProvider.when(\''+route+'\',{template: \''+template+'\'});',exports.ROUTE_MARKER);
    }
...
exports.askForModule = function(type,that,cb){
    ...
    var prompts = [
        {
            name:'module',
            message:'Which module would you like to place the new ' + type + ' into?',
            type: 'list',
            choices: choices,
            default: 0
        }
